...
fn fetch_spot_shadow(light_id: u32, frag_position: vec4<f32>, surface_normal: vec3<f32>) -> f32 {
    ...
    return sample_shadow_map(shadow_uv, depth, i32(light_id) + i32(view_bindings::lights.spot_light_shadowmap_offset), texel_size);
}
...
fn cascade_debug_visualization(
    ...
    let cascade_index = get_cascade_index(light_id, view_z);
    let cascade_color = hsv2rgb(f32(cascade_index) / f32(MAX_CASCADES_PER_LIGHT + 1u), 1.0, 0.5);
    return vec3<f32>(
