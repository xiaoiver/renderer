...
export class SyncSimpleTransforms extends System {
  queries = this.query(
    (q) =>
      q.addedOrChanged.with(Transform).without(Parent, Children).trackWrites,
  );
...
export class PropagateTransforms extends System {
  ...
  execute(): void {
    ...
    this.orphaned_entities.push(...this.orphaned.removed);

    this.root_query.current.forEach((entity) => {
      const transform = entity.read(Transform);
      const children = entity.read(Children);

      this.parent_query.current.forEach((parent) => {});
