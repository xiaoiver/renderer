...
import type {
  ...
  IDecodedData,
  IKTX2DecoderOptions,
} from './ktx2decoderTypes';
import { EngineFormat, TranscodeTarget } from './ktx2decoderTypes';
...
function applyConfig(urls: typeof KhronosTextureContainer2.URLConfig): void {
  ...
  if (urls.wasmUASTCToRGBA_UNORM !== null) {
    KTX2DECODER.LiteTranscoder_UASTC_RGBA_UNORM.WasmModuleURL =
      urls.wasmUASTCToRGBA_UNORM;
  }

  if (urls.wasmUASTCToRGBA_SRGB !== null) {
    KTX2DECODER.LiteTranscoder_UASTC_RGBA_SRGB.WasmModuleURL =
      urls.wasmUASTCToRGBA_SRGB;
  }

  if (urls.wasmUASTCToR8_UNORM !== null) {
    KTX2DECODER.LiteTranscoder_UASTC_R8_UNORM.WasmModuleURL =
      urls.wasmUASTCToR8_UNORM;
  }

  if (urls.wasmUASTCToRG8_UNORM !== null) {
    KTX2DECODER.LiteTranscoder_UASTC_RG8_UNORM.WasmModuleURL =
      urls.wasmUASTCToRG8_UNORM;
  }
  ...
  public _getKTX2DecoderOptions(): IKTX2DecoderOptions {
    ...
    const options: IKTX2DecoderOptions = {
      useRGBAIfASTCBC7NotAvailableWhenUASTC:
        this._useRGBAIfASTCBC7NotAvailableWhenUASTC,
...
export class KhronosTextureContainer2 {
   ...
   */
  public static DefaultNumWorkers =
  ...
  private static _Initialize(numWorkers: number): void {
    if (
      KhronosTextureContainer2._WorkerPoolPromise ||
      KhronosTextureContainer2._DecoderModulePromise
    ) {
      return;
    }
    ...
    const urls = {
      ...
      wasmUASTCToRGBA_UNORM: getAbsoluteUrlOrNull(
        this.URLConfig.wasmUASTCToRGBA_UNORM,
      ),
      wasmUASTCToRGBA_SRGB: getAbsoluteUrlOrNull(
        this.URLConfig.wasmUASTCToRGBA_SRGB,
      ),
      wasmUASTCToR8_UNORM: getAbsoluteUrlOrNull(
        this.URLConfig.wasmUASTCToR8_UNORM,
      ),
      wasmUASTCToRG8_UNORM: getAbsoluteUrlOrNull(
        this.URLConfig.wasmUASTCToRG8_UNORM,
      ),
    ...
    if (
      ...
      KhronosTextureContainer2._DecoderModulePromise = Tools.LoadScriptAsync(
        urls.jsDecoderModule,
      ).then(() => {
      ...
      KTX2DECODER.MSCTranscoder.UseFromWorkerThread = false;
      KTX2DECODER.WASMMemoryManager.LoadBinariesFromCurrentThread = true;
      KhronosTextureContainer2._DecoderModulePromise = Promise.resolve(
    ...
    if (KhronosTextureContainer2._WorkerPoolPromise) {
        ...
        return new Promise((resolve, reject) => {
          workerPool.push((worker, onComplete) => {
            ...
            worker.postMessage({
              ...
              options:
                KhronosTextureContainer2.DefaultDecoderOptions._getKTX2DecoderOptions(),
            });
      ...
      return KhronosTextureContainer2._DecoderModulePromise.then((decoder) => {
        if (KhronosTextureContainer2.DefaultDecoderOptions.isDirty) {
          KTX2DECODER.KTX2Decoder.DefaultDecoderOptions =
            KhronosTextureContainer2.DefaultDecoderOptions._getKTX2DecoderOptions();
        }
        return new Promise((resolve, reject) => {
          decoder
  ...
  protected _createTexture(
    ...
    if (data.errors) {
      throw new Error(
        'KTX2 container - could not transcode the data. ' + data.errors,
      );
    }
    ...
    for (let t = 0; t < data.mipmaps.length; ++t) {
      ...
      if (!mipmap || !mipmap.data) {
        throw new Error(
          'KTX2 container - could not transcode one of the image',
        );
      }
      ...
      if (isUncompressedFormat) {
        ...
        this._engine._uploadDataToTextureDirectly(
          internalTexture,
          mipmap.data,
          0,
          t,
        ...
        this._engine._uploadCompressedDataToTextureDirectly(
          internalTexture,
          data.transcodedFormat,
          mipmap.width,
          mipmap.height,
          mipmap.data,
          0,
      ...
      case 'decode':
        ktx2Decoder
          ...
          .then((data: IDecodedData) => {
            ...
            postMessage(
              { action: 'decoded', success: true, decodedData: data },
              buffers,
            );
          })
          .catch((reason: any) => {
