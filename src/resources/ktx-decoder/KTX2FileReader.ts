...
export class KTX2FileReader {
  ...
  public parse() {
    ...
    const levelReader = new DataReader(
      this._data,
      offsetInFile,
      levelCount * 3 * (2 * 4),
    );
     ...
     */
    const dfdReader = new DataReader(
      this._data,
     ...
     */
    const sgd: IKTX2_SupercompressionGlobalData =
      (this._supercompressionGlobalData = {});

    if (header.sgdByteLength > 0) {
      const sgdReader = new DataReader(
        this._data,
  ...
  public get needZSTDDecoder(): boolean {
    return (
      this._header.supercompressionScheme === SupercompressionScheme.ZStandard
    );
  }
