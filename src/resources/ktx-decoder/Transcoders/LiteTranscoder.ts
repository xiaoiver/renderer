...
export class LiteTranscoder extends Transcoder {
  ...
  protected _loadModule(): Promise<{ module: any }> {
    ...
    this._modulePromise = WASMMemoryManager.LoadWASM(this._modulePath).then(
      (wasmBinary) => {
        return new Promise((resolve) => {
          WebAssembly.instantiate(wasmBinary as ArrayBuffer, {
            env: { memory: this._memoryManager.wasmMemory },
          }).then((moduleWrapper) => {
  ...
  public transcode(
    ...
    return this._loadModule().then((moduleWrapper: any) => {
      ...
      const [textureView, uncompressedTextureView, nBlocks] =
        this._prepareTranscoding(
          width,
          height,
  ...
  protected _prepareTranscoding(
    ...
    if (uncompressedNumComponents !== undefined) {
      uncompressedByteLength =
        width * ((height + 3) >> 2) * 4 * uncompressedNumComponents;
    }

    const texMemoryPages =
      ((nBlocks * 16 +
        65535 +
        (this._transcodeInPlace ? 0 : uncompressedByteLength)) >>
        16) +
      1;

    const textureView = this.memoryManager.getMemoryView(
      texMemoryPages,
      65536,
      nBlocks * 16,
    );
    ...
    const uncompressedTextureView = this._transcodeInPlace
      ...
      : new Uint8Array(
          ...
          65536 + nBlocks * 16,
          uncompressedNumComponents !== undefined
            ? width * height * uncompressedNumComponents
            : uncompressedByteLength,
        );
