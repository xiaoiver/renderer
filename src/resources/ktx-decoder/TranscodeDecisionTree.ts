            ...
            no: {
              ...
              yes: {
                ...
                yes: {
                  ...
                  engineFormat:
                    KTX2.EngineFormat.COMPRESSED_RGBA_PVRTC_4BPPV1_IMG,
                },
                no: {
                  transcodeFormat: KTX2.TranscodeTarget.PVRTC1_4_RGB,
                  engineFormat:
                    KTX2.EngineFormat.COMPRESSED_RGB_PVRTC_4BPPV1_IMG,
                },
              },
              no: {
                transcodeFormat: KTX2.TranscodeTarget.RGBA32,
                  ...
                  no: {
                    ...
                    yes: {
                      ...
                      yes: {
                        transcodeFormat: KTX2.TranscodeTarget.BC3_RGBA,
                        engineFormat:
                          KTX2.EngineFormat.COMPRESSED_RGBA_S3TC_DXT5_EXT,
                      },
                      no: {
                        transcodeFormat: KTX2.TranscodeTarget.BC1_RGB,
                        engineFormat:
                          KTX2.EngineFormat.COMPRESSED_RGB_S3TC_DXT1_EXT,
                      },
                    },
                    no: {
                      cap: 'pvrtc',
...
export class TranscodeDecisionTree {
  ...
  public parseTree(tree: KTX2.IDecisionTree): boolean {
    const node =
      this._textureFormat === KTX2.SourceTextureFormat.UASTC4x4
        ? tree.UASTC
        : tree.ETC1S;
    if (node) {
  ...
  private _parseNode(node: KTX2.INode | KTX2.ILeaf | undefined): void {
    ...
    if (TranscodeDecisionTree._IsLeafNode(node)) {
      ...
      if (node.cap !== undefined) {
        condition =
          condition && !!this._caps[node.cap as keyof typeof this._caps];
      }
      if (node.option !== undefined) {
        condition =
          condition &&
          !!this._options[node.option as keyof typeof this._options];
      }
      ...
      if (node.transcodeFormat !== undefined) {
        if (Array.isArray(node.transcodeFormat)) {
          condition =
            condition &&
            node.transcodeFormat.indexOf(this._transcodeFormat) !== -1;
        } else {
          condition =
            condition && node.transcodeFormat === this._transcodeFormat;
        }
      }
